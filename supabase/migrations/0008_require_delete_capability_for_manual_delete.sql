-- Requires a deletion capability hash for manual share deletion.
-- This closes share-id-only deletion: possession of path ID is no longer sufficient.

alter table public.psbt_shares
  add column if not exists delete_token_hash text;

update public.psbt_shares
set delete_token_hash = lower(
  md5(random()::text || clock_timestamp()::text || id || ':delete-capability-1')
  || md5(random()::text || clock_timestamp()::text || id || ':delete-capability-2')
)
where delete_token_hash is null;

alter table public.psbt_shares
  drop constraint if exists psbt_shares_delete_token_hash_format;

alter table public.psbt_shares
  add constraint psbt_shares_delete_token_hash_format
  check (delete_token_hash ~ '^[0-9a-f]{64}$');

alter table public.psbt_shares
  alter column delete_token_hash set not null;

drop function if exists public.create_psbt_share(text, text, integer, smallint, timestamptz);
...
drop function if exists public.delete_psbt_share_by_id(text);
